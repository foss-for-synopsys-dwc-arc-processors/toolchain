...
# Specify the default versions to use as a string
# <remote>:<tool>:<branch>. These are the development versions for the ARC 4.8
# tool chain release. Only actually matters if --auto-checkout and/or
# auto-pull is set. <remote> is by default empty, so will fetch from the
# current remote (which is probably what is wanted), but can be set to be an
# explicit named remote if necessary.
...
do
    remote=`echo ${version} | cut -d ':' -f 1`
    tool=`echo ${version} | cut -d ':' -f 2`
    branch=`echo ${version} | cut -d ':' -f 3`
	...
	# this if we have auto-pull enabled (so we work if not online).
	if ! git fetch ${remote}
	then
	    exit 1
	fi
